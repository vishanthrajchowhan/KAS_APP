...
def generate_fallback_walkthrough_report(transcript: str, job: dict | None = None, photo_timeline=None, field_notes: str = "", error: str = "") -> dict:
    ...
    if error:
        summary_parts.append("Automated AI enhancement was unavailable, so this report was generated from captured notes, transcript, and photos.")
...
def generate_pdf_from_report(html_content: str, out_pdf_path: str) -> None:
    if HTML is None:
        ...
        HTML(string=html_content, base_url=str(BASE_DIR)).write_pdf(out_pdf_path)
...
def generate_pdf_with_reportlab(html_content: str, out_pdf_path: str) -> None:
    ...
    class ReportTextExtractor(HTMLParser):
        block_tags = {"address", "article", "br", "div", "footer", "h1", "h2", "h3", "h4", "h5", "h6", "header", "li", "p", "section", "tr"}
    ...
    for line in content.split('\n'):
        ...
        if not line:
            ...
            p = Paragraph(html.escape(line), styles['Normal'])
            story.append(p)
...
def get_walkthrough_and_job(walkthrough_id):
    with get_db_connection() as conn:
        walkthrough = conn.execute("SELECT * FROM walkthroughs WHERE id = ?", (walkthrough_id,)).fetchone()
        if walkthrough is None:
            return None, None
        job = conn.execute("SELECT * FROM jobs WHERE id = ?", (walkthrough["job_id"],)).fetchone()
    return walkthrough, job
...
def api_walkthrough_status(walkthrough_id: int):
    ...
    row, job = get_walkthrough_and_job(walkthrough_id)
    if not row:
...
def upload_walkthrough_pdf(walkthrough_id: int):
    ...
    access_row, access_job = get_walkthrough_and_job(walkthrough_id)
    if not access_row:
        flash("Walkthrough not found.", "error")
        return redirect(url_for("index"))
    if not access_job or not can_update_job(access_job):
        abort(403)
